...
description: Göreve dönük Wings tarifleri — CRUD API'ler, auth middleware, dependency injection, JWT authentication, validation, sayfalama, dosya yükleme, statik dosyalar, route grupları, response model, cache, dosya indirme ve SQLite destekli API'ler.
---
...
## JWT authentication nasıl eklerim?

Gerçek bir login akışı için — token'ı bir kez üret, sonra diğer tüm route'ları
onunla koru — [`wings_jwt`](https://api.pkg.tulparlang.dev) paketini (token
...
`jwt_guard`, global bir `use()` middleware'i bağlar, yani kayıtlı **her**
