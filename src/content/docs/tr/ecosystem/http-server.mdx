...
Dizin-tarzı bir istek (mount kökü veya `/` ile biten yol) `index.html` sunar,
...
Varsayılan `200 application/json` + CORS header'ları çoğu durumu karşılar. Düz
...
:::caution
Handler'dan `return http_create_response(...)` yapma. Bu builtin **tam bir HTTP
wire string'i** döndürür, ama dispatcher handler'ın dönüş değerini serialize
edilecek veri sayar — yani string tekrar JSON olarak sarılır. Yukarıdaki `_raw`
envelope'unu ya da aşağıdaki streaming desenini kullan.
:::

Ayrıca **özel header'lara** ihtiyacın olduğunda — örneğin `Content-Disposition`
...
`http_create_response(status, content_type, body, headers, keep_alive)` alttaki
