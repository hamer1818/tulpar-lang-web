...
`socket_peer_ip(fd)` returns the remote (client) IP of an accepted
connection — handy for logging or rate limiting. It returns `""` on error
(bad fd, unconnected socket). The Wings/router stack already exposes it on the
request as `_request["remote_addr"]`.

```tulpar
int client = socket_accept(sockfd);
str ip = socket_peer_ip(client);   // e.g. "127.0.0.1"
